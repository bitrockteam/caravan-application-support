module "jaeger" {
  ...
  source                                   = "git::https://github.com/bitrockteam/caravan-cart//modules/jaeger?ref=refs/tags/v0.4.5"
...
module "filebeat" {
  count           = var.configure_monitoring ? 1 : 0
  source          = "git::https://github.com/bitrockteam/caravan-cart//modules/filebeat?ref=refs/tags/v0.3.7"
  dc_names        = var.dc_names
  domain          = var.domain
  services_domain = var.services_domain
}
...
resource "nomad_job" "consul-terminating" {
  ...
  hcl2 {
    enabled = true
    vars = {
      datacenters          = jsonencode(var.dc_names)
      terminating_services = jsonencode(var.terminating_services)
      dns_servers          = jsonencode([var.nameserver_dummy_ip])
    }
  }
